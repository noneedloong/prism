...
final class AppSettings {
    ...
    var conversationMode: ConversationMode = .balanced {
        didSet { saveConfig() }
    }
    ...
    /// - macOS: ~/Documents/Prism (shares data with the GUI app)
    /// - Linux: ~/.local/share/prism (XDG_DATA_HOME convention)
    /// - Windows: %APPDATA%/Prism
    private static let localDefaultPath: String = {
        let home = FileManager.default.homeDirectoryForCurrentUser
        #if os(macOS)
        return home.appendingPathComponent("Documents/Prism").path
        #elseif os(Windows)
        return home.appendingPathComponent("AppData/Roaming/Prism").path
        #else
        // Linux / other Unix: XDG_DATA_HOME or ~/.local/share
        if let xdg = ProcessInfo.processInfo.environment["XDG_DATA_HOME"], !xdg.isEmpty {
            return URL(fileURLWithPath: xdg).appendingPathComponent("prism").path
        }
        return home.appendingPathComponent(".local/share/prism").path
        #endif
    }()
